...
(def event-pull-pattern
  '[:provider-event/id :provider-event/provider :provider-event/external-id
    ...
    :provider-event/provider-reference :provider-event/received-at :provider-event/error])
...
(defn- event->tx [event]
  {:provider-event/id (:provider-event/id event)
...
(defrecord DatomicProviderEventRepository [connection]
  ...
  (payment-by-provider-reference [_ provider reference]
    (when-let [payment-eid
               (ffirst (d/q '[:find ?payment
